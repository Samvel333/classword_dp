...
class User {
    ...
    static create = function (req, res) {
        try {
            ...
            connection.query('INSERT INTO `users` (`name`,`gender`,`dob`) VALUES (req.body.name, req.body.gender, req.body.dob)',
    ...
    static update = function (req, res) {
        try {
            const errors = validationResult(req)
            connection.query('UPDATE users SET name = ?, gender = ?, dob = ? WHERE id = ${req.params.id}',
                [req.body.name, req.body.gender, req.body.dob], (err) => {
                    if (err) throw err;
                })
            if (!errors.isEmpty()) {
                return res.status(400).json({errors: errors.array()});
